// $Id: thickbox.js,v 1.6 2010/08/31 14:40:49 jozik Exp $
...
// Initialize Thickbox.
if (Drupal.settings.icl_D7 !== undefined) {
...
function iclThickboxReopen(url) {
  ...
  $('#iclThickboxReopenLink').remove();
  $('body').prepend('<a id="iclThickboxReopenLink" href="'+url+glue+'keepThis=true&amp;TB_iframe=true" class="thickbox" style="display:none;">test</a>');
  icl_tb_set_size('#iclThickboxReopenLink');
  $('#iclThickboxReopenLink').addClass('initThickbox-processed').click(function() {
    var t = this.title || this.name || null;
    var a = this.href || this.alt;
    var g = this.rel || false;
    tb_show(t,a,g);
  ...
  window.setTimeout(function() {
    $('#iclThickboxReopenLink').trigger('click');
